...
if __name__ == '__main__':
    ...
    times_q = [i * (sim_duration - 2 * prerun) // 5 + (prerun if i > 0 else 0) for i in range(4)]

    time_stim, q_stc_hist = simulate_with_trajectory(
        EI_Network,
        nest,
        q_stc_initial=q_stc,
        warmup=warmup,
    ...
    # ----- adaptation per bin from time_stim/q_stc_hist -----
    time_stim = np.asarray(time_stim, float)
    q_stc_hist = np.asarray(q_stc_hist, float)
    assert len(time_stim) == len(q_stc_hist), "time_stim and q_stc_hist must have same length"
    adapt_idx = np.searchsorted(time_stim[1:], t_ms, side='right')  # 0..K-1
    adapt_q = q_stc_hist[adapt_idx]
